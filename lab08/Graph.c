...
Graph GraphMST(Graph g) {
    ...
    Graph mst = GraphNew(g->nV); // initially an empty spanning tree
    PQ queue = PQNew(); // create an empty queue

    // i nt storeEdge[g->nV];

    for (int i = 0; i < g->nE; i++) {
        for (int j = 1; j < g->nE; j++) {
            if (g->edges[i][j] != 0) {
                PQInsert(queue, (Edge){i, j, g->edges[i][j]});
            }
        } // store edges in array (but Edge is taken in this case isntead of array)
    }
    for (int i = 0; mst->nE < g->nV - 1; i++) {
		Edge e = PQExtract(queue);
